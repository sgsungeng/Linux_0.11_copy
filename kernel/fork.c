...
int copy_mem(int nr,struct task_struct * p) // set the ldt info to task p
{
	...
	code_limit=get_limit(0x0f); // 1 index 1， 1 local descriptor， 11 user mode ， code segment
	data_limit=get_limit(0x17); // 10 index 2，1 11 data segment
		...
		long eip,long cs,long eflags,long esp,long ss)
{
	...
	*p = *current;	/* NOTE! this doesn't copy the supervisor stack */
	...
	p->tss.eip = eip;
	p->tss.eflags = eflags;
	p->tss.eax = 0;  //zero for sub progres return value
	p->tss.ecx = ecx;
	p->tss.edx = edx;
	p->tss.ebx = ebx;
	p->tss.esp = esp;
